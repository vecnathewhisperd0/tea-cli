...
func CreateLogin(name, token, user, passwd, sshKey, giteaURL, sshCertPrincipal, sshKeyFingerprint, oauthRefreshToken string, insecure, sshAgent, versionCheck bool, tokenExpiry time.Time) error {
	// checks ...
	// ... if we have a url
	if len(giteaURL) == 0 {
